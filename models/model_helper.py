...
class Model:
    ...
    def load_model(self,model_type, K=1, F=8, additional_params={}):
        ...
        if model_type == "MPNN":
            self.model = GNN_MPNN_Model(hidden_size=self.F, input_size=self.dataset_num_features, output_size=self.dataset_num_classes, num_layers=self.K)
        elif model_type == "SAGE":
            self.model = GNN_SAGE_Model(hidden_size=self.F, input_size=self.dataset_num_features, output_size=self.dataset_num_classes, num_layers=self.K)
        elif model_type == "GAT":
            self.model = GNN_CONV_Model(hidden_size=self.F, input_size=self.dataset_num_features, output_size=self.dataset_num_classes, num_layers=self.K)
        elif model_type == "GCN":
            #self.model = GAT(in_channels=self.dataset_num_features, hidden_channels=self.F, num_layers=self.K, out_channels=self.dataset_num_classes)
            self.model = GNN_GAT_Model(hidden_size=self.F, input_size=self.dataset_num_features, output_size=self.dataset_num_classes, num_layers=self.K, n_heads=1)
        elif model_type == "GIN":
